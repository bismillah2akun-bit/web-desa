SET NAMES utf8mb4;
CREATE TABLE IF NOT EXISTS village_profile (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,name VARCHAR(150) NOT NULL,district VARCHAR(120),regency VARCHAR(120),province VARCHAR(120),postal_code VARCHAR(10),area_size_ha DECIMAL(12,2),hamlet_count INT UNSIGNED,boundary_north VARCHAR(255),boundary_east VARCHAR(255),boundary_south VARCHAR(255),boundary_west VARCHAR(255),history TEXT,vision TEXT,mission TEXT,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB;
CREATE TABLE IF NOT EXISTS news (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,title VARCHAR(255) NOT NULL,slug VARCHAR(255) UNIQUE NOT NULL,category VARCHAR(80),summary TEXT,content LONGTEXT,image_url TEXT,image_urls JSON NULL,is_published BOOLEAN DEFAULT TRUE,published_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS administrative_areas (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,rw_number VARCHAR(10) NOT NULL,rt_number VARCHAR(10) NOT NULL,household_count INT UNSIGNED,male_population INT UNSIGNED,female_population INT UNSIGNED,data_year SMALLINT UNSIGNED,source TEXT,status VARCHAR(30) DEFAULT 'belum_diverifikasi',photo_url TEXT NULL,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,UNIQUE KEY uq_rw_rt(rw_number,rt_number)) ENGINE=InnoDB;
CREATE TABLE IF NOT EXISTS neighborhood_officials (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,level ENUM('rw','rt') NOT NULL,number VARCHAR(10) NOT NULL,name VARCHAR(180) NOT NULL,photo_url TEXT NULL,sort_order INT UNSIGNED DEFAULT 0,is_active BOOLEAN DEFAULT TRUE,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,UNIQUE KEY uq_neighborhood_official(level,number)) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS service_requirements (
  ...
  template_stored_name VARCHAR(255) NULL,
...
INSERT INTO village_profile(name,district,regency,province,postal_code,history,vision,mission,welcome_title,welcome_message) SELECT 'Desa Tanjungjaya','Cihampelas','Bandung Barat','Jawa Barat','40562','Wilayah Tanjungjaya semula merupakan bagian dari Desa Rancairung. Pemekaran disepakati pada 17 November 1972.','Data visi akan diperbarui oleh admin desa.','Data misi akan diperbarui oleh admin desa.','Bersama membangun desa yang terbuka dan berdaya','Selamat datang di portal Desa Tanjungjaya. Website ini disiapkan sebagai ruang informasi, pengenalan potensi, dan akses layanan bagi warga.' WHERE NOT EXISTS(SELECT 1 FROM village_profile);
